...
export const readPacientesTool: ToolDefinition = {
  definition: {
    name: "read_pacientes",
  ...
  execute: async (input) => {
    try {
      ...
      const result = await patientsService.list({ search, page, pageSize: 10 });
...
export const readCitasTool: ToolDefinition = {
  definition: {
    name: "read_citas",
  ...
  execute: async (input) => {
    try {
      ...
      const doctorId = typeof input["doctorId"] === "string" ? input["doctorId"] : undefined;
      ...
      const appointments = await appointmentsService.listByDateRange(start, end, doctorId);
      return JSON.stringify({
...
export const readInventarioTool: ToolDefinition = {
  definition: {
  ...
  execute: async (input) => {
    try {
      const search = typeof input["search"] === "string" ? input["search"] : undefined;
      const items = await inventoryService.list({ search, active: true });
      return JSON.stringify({
...
export const readCajaTool: ToolDefinition = {
  definition: {
    name: "read_caja",
  ...
  execute: async (input) => {
    try {
      ...
      const page = typeof input["page"] === "number" ? input["page"] : 1;
      const result = await cajaService.list({ search, status, page });
      return JSON.stringify({
