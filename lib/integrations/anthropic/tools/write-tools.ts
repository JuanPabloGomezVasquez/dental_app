...
import type { AgentContext } from "./read-tools";
...
export function makeWriteTools(ctx: AgentContext): WriteToolDefinition[] {
  return [
    {
      requiresConfirmation: true,
      definition: {
      ...
      execute: async (input) => {
        try {
          ...
          const patient = await patientsService.create(parsed.data, ctx.organizationId);
    ...
    },
    {
      ...
      execute: async (input) => {
        try {
          ...
          if (!parsed.success) return JSON.stringify({ error: parsed.error.issues[0]?.message ?? "Datos inválidos" });
          const appointment = await appointmentsService.create(parsed.data, ctx.organizationId);
    ...
    },
    {
      ...
      execute: async (input) => {
        try {
          ...
          if (!parsed.success) return JSON.stringify({ error: parsed.error.issues[0]?.message ?? "Datos inválidos" });
          const item = await inventoryService.updateStock(id, ctx.organizationId, parsed.data);
    ...
    },
    {
      ...
      execute: async (input) => {
        try {
          ...
          if (!parsed.success) return JSON.stringify({ error: parsed.error.issues[0]?.message ?? "Datos inválidos" });
          const record = await cajaService.create(parsed.data, ctx.organizationId);
    ...
    },
    {
      ...
      execute: async (input) => {
        try {
          ...
          if (!parsed.success) return JSON.stringify({ error: parsed.error.issues[0]?.message ?? "Datos inválidos" });
          const result = await cajaService.addPayment(cajaRecordId, ctx.organizationId, parsed.data);
    ...
    },
    {
      ...
      execute: async (input) => {
        try {
          ...
          if (!parsed.success) return JSON.stringify({ error: parsed.error.issues[0]?.message ?? "Datos inválidos" });
          const doctor = await doctorsService.create(parsed.data, ctx.organizationId);
    ...
    },
    {
      ...
      execute: async (input) => {
        try {
          ...
          if (!parsed.success) return JSON.stringify({ error: parsed.error.issues[0]?.message ?? "Datos inválidos" });
          const procedure = await proceduresService.create(parsed.data, ctx.organizationId);
